...
[https://github.com/anniedong7/Dong_Annie_201_A06ithub-repo-url](https://github.com/anniedong7/Dong_Annie_201_A06)
...
```{r}
num_index <- player_info[[2]][[4]]
num_index

name_index <- player_info$LJ$points
name_index
```
...
for(i in names(player_info)){
  ...
  points_game <- points / games
  points_per_game[[i]] <- points_game
}
...
```{r}
round_values <- sapply(points_per_game, round, digits = 2)
round_values
```
...
round_mat <- apply(mat_rand, MARGIN = 1:2, FUN = round_val)
...
max_idx <- which.max(annual_temps)
min_idx <- which.min(annual_temps)

temp_max <- annual_temps[max_idx]
temp_min <- annual_temps[min_idx]

print(paste0('The maximum median annual temperature of ', temp_max, ' occurred in ' , CO$year[max_idx] , ' and the minimum median annual temperature of ', temp_min, ' occurred in ', CO$year[min_idx], '.'))
```
